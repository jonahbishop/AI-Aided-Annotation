...
def test_mmr_cloud():
  ...
  sents = mmr.process_file(sessionID, sents)
...
# Maybe this should be a real method in server.py
def make_chap_from_text(text):
  """"""
  sents = mmr.tokenize_sentences(text)
  keywr = mmr.keyword_generator(sents)
  return {"full_text": text, "sentences": sents, "keywords":keywr}


def check_chapter(sessionID, chap_num, chap_data):
  for key, val in chap_data.items():
    print(key)
    db_val = db.chap_get(sessionID, chap_num, key)
    print(db_val)
    if key == "keywords":
      for tup, db_tup in zip(val, db_val):
        assert(tup[0] == db_tup[0])
        assert(tup[1] == db_tup[1])
    else:
      assert(val == db_val)
...
def test_mongo():
  ...
  assert(db.session_num_chapters(sessionID) == 0)

  db.session_add_chapter(sessionID, chap0)
  assert(db.session_num_chapters(sessionID) == 1)
  check_chapter(sessionID, 0, chap0)  
    
  db.session_add_chapter(sessionID, chap1)
  assert(db.session_num_chapters(sessionID) == 2)
  check_chapter(sessionID, 1, chap1)

  db.session_set_chapter(sessionID, 0, chap1)
  assert(db.session_num_chapters(sessionID) == 2)
  check_chapter(sessionID, 0, chap1)

  db.session_add_chapter(sessionID, chap0)
  assert(db.session_num_chapters(sessionID) == 3)
  check_chapter(sessionID, 2, chap0)
